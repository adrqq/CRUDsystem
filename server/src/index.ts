...
//       // mainData.data = eventsArray;
//       // mainData._id = i;

//       // await events.insertOne(mainData);
//       // mainData = {};
...
app.use('/', (req, res, next) => {
  ...
  app.get('/users', (req: Request, res: Response) => {
    ...
    const getUsers = async () => {
      try {
        ...
        // const usersData = await users.find().sort({ _id: -1 }).skip((Number(_page) - 1) * Number(_limit)).limit(Number(_limit)).toArray();
  ...
  app.post('/events', (req: Request, res: Response) => {
    const { name, description, date, startTime, endTime, userid } = req.body;
